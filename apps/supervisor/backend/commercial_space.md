...
> `<catalog>.<schema>.*`. Every column below exists in `data/output/commercial/README.md` —
...
|---|---|---|---|
| `<catalog>.<schema>.accounts` | Customer / account master | `account_id`, `account_name`, `region` (EMEA/Americas/APAC/China), `segment` (Architectural / Industrial / Marine & Protective / Automotive Refinish), `industry`, `owner_rep` | one row per account |
| `<catalog>.<schema>.pipeline` | Open & closed opportunities | `opp_id`, `account_id`, `stage`, `amount_eur`, `close_month` (DATE), `product_line` (Decorative Paints \| Performance Coatings) | one row per opportunity |
| `<catalog>.<schema>.sales_actuals` | Realized sales per account | `account_id`, `month` (DATE), `revenue_eur`, `volume_units`, `margin_eur` | account × month |
| `<catalog>.<schema>.churn_signals` | Monthly churn-risk signals | `account_id`, `month`, `churn_score` (0–1), `last_order_days`, `complaint_count`, `nps` | account × month |
...
FROM <catalog>.<schema>.churn_signals c
JOIN <catalog>.<schema>.accounts a ON c.account_id = a.account_id
...
FROM <catalog>.<schema>.sales_actuals s
...
JOIN <catalog>.<schema>.pipeline p ON p.account_id = a.account_id
...
FROM <catalog>.<schema>.sales_actuals s
JOIN <catalog>.<schema>.accounts a ON s.account_id = a.account_id
...
FROM <catalog>.<schema>.pipeline p
JOIN <catalog>.<schema>.accounts a ON p.account_id = a.account_id
WHERE a.region = 'EMEA'
...
WITH q AS (
  SELECT account_id,
    ...
    revenue_eur
  FROM <catalog>.<schema>.sales_actuals
...
FROM q JOIN <catalog>.<schema>.accounts a ON q.account_id = a.account_id
...
FROM <catalog>.<schema>.churn_signals c
JOIN <catalog>.<schema>.sales_actuals s
