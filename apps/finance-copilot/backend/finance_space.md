...
> against `<catalog>.<schema>.*`. Do not invent columns — every column below exists in
...
|---|---|---|---|
| `<catalog>.<schema>.products` | SKU master / product dimension | `sku`, `product_name`, `product_line` (Decorative Paints \| Performance Coatings), `region` (EMEA/Americas/APAC/China), `currency`, `list_price_eur`, `standard_cost_eur` | one row per SKU |
| `<catalog>.<schema>.margin_actuals` | Realized monthly P&L per SKU×region | `sku`, `region`, `month` (DATE, first-of-month), `units`, `revenue_eur`, `cogs_eur`, `gross_margin_eur`, `gross_margin_pct` (fraction, 0.42 = 42%) | sku × region × month |
| `<catalog>.<schema>.margin_budget` | Plan / budget figures (no shocks) | `sku`, `region`, `month`, `budget_units`, `budget_revenue_eur`, `budget_margin_eur` | sku × region × month |
| `<catalog>.<schema>.fx_rates` | Monthly FX to EUR | `currency` (EUR/USD/GBP/CNY), `month`, `rate_to_eur` (EUR=1.0) | currency × month |
| `<catalog>.<schema>.cost_drivers` | COGS decomposition | `sku`, `region`, `month`, `raw_material_cost`, `freight_cost`, `energy_cost`, `overhead` (EUR; sum reconciles to `cogs_eur`) | sku × region × month |
...
WITH base AS (
  ...
  FROM <catalog>.<schema>.margin_actuals m
  JOIN <catalog>.<schema>.products p ON m.sku = p.sku
  LEFT JOIN <catalog>.<schema>.cost_drivers c
...
SELECT
  ...
  ROUND(SUM(gross_margin_eur) / SUM(revenue_eur) * 100, 1) AS gross_margin_pct
FROM <catalog>.<schema>.margin_actuals
...
FROM <catalog>.<schema>.cost_drivers c
JOIN <catalog>.<schema>.products p ON c.sku = p.sku
WHERE p.product_line = 'Decorative Paints' AND p.region = 'EMEA'
...
FROM <catalog>.<schema>.margin_actuals m
JOIN <catalog>.<schema>.products p ON m.sku = p.sku
JOIN <catalog>.<schema>.margin_budget b
