...
> `serverless_lakebase_praneeth_catalog.akzo_scm.*`. Every column below exists in `data/output/scm/README.md` — do not invent columns.
...
All tables fully-qualified under `serverless_lakebase_praneeth_catalog.akzo_scm`.

| Table | Purpose | Key columns | Grain |
|---|---|---|---|
| `serverless_lakebase_praneeth_catalog.akzo_scm.otif` | On-Time-In-Full delivery performance | `plant`, `region` (EMEA/Americas/APAC/China), `lane`, `sku`, `month` (DATE), `orders`, `on_time`, `in_full`, `otif_pct` | plant × region × lane × sku × month |
| `serverless_lakebase_praneeth_catalog.akzo_scm.inventory` | Inventory health & stockouts | `plant`, `sku`, `month`, `on_hand_units`, `safety_stock`, `days_of_supply`, `stockout_flag` (1=stockout) | plant × sku × month |
| `serverless_lakebase_praneeth_catalog.akzo_scm.lanes` | Lane master (current values) | `lane_id`, `origin_plant`, `dest_region`, `mode` (road/sea/air), `lead_time_days` (current/Q2 2026), `cost_per_unit` (EUR) | one row per lane |
| `serverless_lakebase_praneeth_catalog.akzo_scm.service_levels` | Regional customer service | `region`, `month`, `service_pct`, `backorder_units` | region × month |
...
- To make this **"Paints EMEA"**, filter `otif.region = 'EMEA'` and SKUs that are Decorative Paints. The Decorative EMEA SKUs are `DEC-1000, DEC-1004, DEC-1008, DEC-1012, DEC-1016, DEC-1020, DEC-1024, DEC-1028`; or join to `serverless_lakebase_praneeth_catalog.akzo_finance.products` on `sku` and filter `product_line='Decorative Paints'` for the authoritative list (cross-catalog read, same SKU ids).
...
SELECT o.month,
       ...
       s.backorder_units
FROM o
JOIN serverless_lakebase_praneeth_catalog.akzo_scm.service_levels s
...
WITH mode_norm AS (
  ...
  FROM serverless_lakebase_praneeth_catalog.akzo_scm.lanes GROUP BY mode
)
...
FROM serverless_lakebase_praneeth_catalog.akzo_scm.lanes l
...
FROM serverless_lakebase_praneeth_catalog.akzo_scm.inventory i
...
FROM serverless_lakebase_praneeth_catalog.akzo_scm.service_levels
...
FROM serverless_lakebase_praneeth_catalog.akzo_scm.lanes
...
FROM serverless_lakebase_praneeth_catalog.akzo_scm.inventory
